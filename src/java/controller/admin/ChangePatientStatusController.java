 ...
 * @author ADMIN
 */
@WebServlet(name="ChangePatientStatusController", urlPatterns={"/changePatientStatus"})
public class ChangePatientStatusController extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            boolean success = new PatientDAO().updatePatientStatus(id, status);
            String message = success ? "Thay đổi trạng thái thành công" : "Thay đổi trạng thái thất bại";
            response.sendRedirect(request.getContextPath() + "/viewPatientDetail?id=" + id + "&message=" + java.net.URLEncoder.encode(message, "UTF-8"));
        } catch (Exception e) {
            e.printStackTrace();
            response.sendRedirect(request.getContextPath() + "/managePatient");
        }
